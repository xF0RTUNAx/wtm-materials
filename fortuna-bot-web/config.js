const CONFIG = {
  SUPABASE_URL: "https://bhcvxunvbxodkpcrjwbq.supabase.co",
  SUPABASE_ANON_KEY:
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJoY3Z4dW52YnhvZGtwY3Jqd2JxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODkzNTIzNDQsImV4cCI6MjEwNDkyODM0NH0.GLhVO5uv2vjSVcGyflOLYVRs6TytXOWZDqRpDGAbhN4",

  AUTH_URL: "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/auth-password",
  PLAYER_STATE_URL:
    "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/get-player-state",
  CLAIM_CODE_URL:
    "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/claim-migration-code",

  FARM_FIREBALL_URL:
    "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/farm-fireball",
  FARM_RADIOFUGAS_URL:
    "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/farm-radiofugas",
  FARM_LOOT_URL:
    "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/farm-loot",
  FARM_MELADZE_URL:
    "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/farm-meladze",

  SHOP_BUY_URL: "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/shop-buy",
  CONTAINER_OPEN_URL:
    "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/container-open",

  EQUIPMENT_CRAFT_URL:
    "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/equipment-craft",
  EQUIPMENT_EQUIP_URL:
    "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/equipment-equip",

  RAID_STATUS_URL: "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/get-raid-status",
  RAID_BUY_WEAPON_URL:
    "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/raid-buy-weapon",
  RAID_ATTACK_URL: "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/raid-attack",
  RAID_START_URL: "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/raid-start",
  RAID_STOP_URL: "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/raid-stop",

  MINIGAME_SPIN_URL: "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/minigame-spin",

  PLAYER_PROFILE_URL:
    "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/get-player-profile",

  LEADERBOARDS_URL:
    "https://bhcvxunvbxodkpcrjwbq.supabase.co/functions/v1/get-leaderboards",
};
